...
public class ManagerEmployee {
	...
	public ManagerEmployee() throws ClassNotFoundException, IOException {
		...
		for (Employee employee : listEmployee) {
			String typeString = "Thợ Máy";
			if(employee.getType() == 0)
				typeString = "Quản Lí";
			else if(employee.getType() == 1)
				typeString = "Thu Ngân";
			Object[] row = {employee.getID(), employee.getName(), employee.getGender(), employee.getPhone(),employee.getAddress(),employee.getSalary(),typeString};
			dtm.addRow(row);
		}
		...
		btnAddEmployee.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(tfIDEmployee.getText().equals("")
				...
				} else if(tfPhoneEmployee.getText().toString().length() > 10) {
					JOptionPane.showMessageDialog(null, "Số điện thoại không lớn hơn 10 số",
		  	                  "Lỗi", JOptionPane.ERROR_MESSAGE);
				}
				else {
					...
					if(cbCVEmployee.getSelectedItem().toString().equals("Quản Lí"))
						employee.setType(0);
					else if(cbCVEmployee.getSelectedItem().toString().equals("Thu Ngân"))
						employee.setType(1);
					else if(cbCVEmployee.getSelectedItem().toString().equals("Thợ Máy"))
						employee.setType(2);
